...
CREATE TABLE dbconnect4.userdeleted(
	...
	CONSTRAINT FK_USERDELETED_1 FOREIGN KEY (EMAIL_REMOVER) REFERENCES dbconnect4.user(EMAIL) ON DELETE CASCADE,
	PRIMARY KEY (EMAIL)
);
	...
	BEGIN
		...
		ELSE
			IF  EXISTS(SELECT NULL FROM correct_size(R:=new.ROW_SIZE,C:=new.COL_SIZE)) THEN
				IF NOT EXISTS (SELECT NULL FROM dbconnect4.game WHERE (new.D_INIT>D_INIT AND new.D_INIT<D_END) OR (new.D_END>D_INIT AND new.D_END<D_END)) THEN
					RETURN new;
				ELSE 
